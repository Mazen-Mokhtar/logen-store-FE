...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendUrl = `${BACKEND_API_URL}/v1/auth/profile`;
    
    console.log('Proxying profile request to backend:', backendUrl);
    ...
    console.log('Backend profile response received:', { 
      success: data.success, 
      user: data.data?.email || 'N/A' 
    });
